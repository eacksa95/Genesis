...
/**
 *
 * @author Gregorio Quintana González 2022
 */
public class tableModel {
    ...
    /**
     * Constructor que recibe el nombre de la tabla a la que representará la
     * clase
     *
     * @param tableName
     */
    ...
    public int saveRegister(Map<String, String> viewRegister) {
        ...
        sql += this.createKeyListString(keys); //nombre de columnas separadas por ", "
        sql += ") VALUES (";
        sql += this.createValueListString(viewRegister, ","); //valores para las columnas formateados ", "
        sql += ")";                                           
    ...
    /**
     * Prepara y ejecuta sentencia sql para actualizar (UPDATE) los datos en la
     * tabla
     *
     * @param viewRegister Map que contiene el par campo-valor de los campos que
     * no son clave primaria
     * @param conditions Map que contiene el par campo-valor de los campos clave
    ...
    public int deleteRegister(Map<String, String> keys) {
        ...
        for (String key : this.tableKeys) {          //Recorre todos los primary key
            String v; //Recupera el valor correspondiente al key, sino existe devuelve una cadena vacía ""
            v = keys.get(key); //getOrDefault(key, "");
            v = (v == null) ? "" : v;
            if (!v.equals("")) {                     //Si no está vacía entonces
                aRow = new ArrayList<String>();    //se crea una nueva instancia de array con elementos tipo String
                aRow.add(key);                     //donde se ingresa como elemento 0 el id
                aRow.add("=");                     //segundo elemento el comparador   
                aRow.add(keys.get(key));           //tercer elemento el valor
                aList.add(aRow);                   //y se almacena en el array de array
            }
    ...
    /**
     * consultar registros filtrados por condicional
     * SQL Select (fieldsToSelect) where (conditions)
     * @param fieldsToSelect
     * @param conditions
     * @return rtn
     */
    public ArrayList<Map<String, String>> readRegister(Map<String, String> fieldsToSelect, Map<String, String> conditions) {
        ArrayList<Map<String, String>> rtn = new ArrayList<>();
        ResultSet rs;
        //Sólo necesitamos la lista de campos a poner en el select
        ArrayList fields = new ArrayList(fieldsToSelect.keySet());
        String sql;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= colCount; i++) {
                    ...
                    String value = conditions.get(columnName);
                    //................................................
            ...
            System.out.println("tableModel 186 readRegister SQLException");
        }
        return rtn;
    }

    //realiza un select fieldsToselect from tbModel where conditions
    public ResultSet readRegisterResultSet(Map<String, String> fielsToSelect, Map<String, String> conditions) {
        ResultSet rs;
        //Sólo necesitamos la lista de campos a poner en el select
    ...
    public ArrayList<Map<String, String>> readRegisterList(Map<String, String> fielsToSelect, Map<String, String> conditions) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= colCount; i++) {
                    System.out.println("tableModel 217" + metaData.getColumnLabel(i));
                    System.out.println("tableModel 218" + rs.getString(i));
                    mapColumns.put(metaData.getColumnLabel(i), rs.getString(i));
                }
                rtn.add(mapColumns);
            }
    ...
    //crea un string con los nombres de los campos del modelo separados por comas ","
    //se utiliza para crear sentencias sql ejemplo: insert into usuarios(createKeyListString()) values().....
    ...
    public String matchValueType(String key, String value) {
        ...
        if (this.columnNames.contains(key)) {
            index = this.columnNames.indexOf(key);
            fieldType = this.columnTypes.get(index);
            System.out.println("tableModel 615 matchValueType: clave "+key+" valor "+value+" tipo "+fieldType);
            switch (fieldType) {
                case "int":
    ...
    /**
     * *
     * Método verifica que solo queden aquellas claves del Map que se corresponden
     * con los campos de la tabla de la bd. Puede pedirse sólo las prímary key
     *
     * @param myMap Map que se envía desde la vista, puede tener claves extras
     * @return rtn Map de claves que son iguales a los campos de la tabla. Puede
     * que no sea la totalidad de los campos.
     */
